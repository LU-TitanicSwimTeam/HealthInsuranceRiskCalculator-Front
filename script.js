document.addEventListener("DOMContentLoaded", function () {
  ...
  const API_BASE_URL = "https://healthinsuranceriskcalc-back-cgcsbha9dsaydnhp.centralus-01.azurewebsites.net";
  ...
  document.getElementById("riskForm").addEventListener("submit", function (event) {
      ...
      if (!formData.age || !formData.weight || !formData.heightFeet || !formData.heightInches || !formData.systolic || !formData.diastolic) {
          document.getElementById("results").innerHTML = `<p class="error">Please fill out all fields.</p>`;
          return;
      }

      fetch(`${API_BASE_URL}/api/calculate-risk`, {
          method: "POST",
